...
#include <intrin.h>
#include <uc_dev/sys/spin_lock.h>
    ...
    namespace mem
    {
        namespace streamflow
        {
                ...
                page_block* alllocate(std::size_t size) throw()
                {
                    ...
                    if (buddy != nullptr)
                    {
                        ...
                        while ( k > order)
                        {
                            ...
                            uintptr_t  s                    =   ( page_size * ( 1 << k ) ) ;  
                            uintptr_t  right_half_address   =   buddy_address + s;
            ...
            class radix_page_map : private detail::noncopyable
            {
                ...
                inline node* allocate_node() throw()
                {
                    return reinterpret_cast<node*> ( m_allocator->allocate( sizeof(node) ) );
                }

                inline void free_node(node* node)  throw()
                {
                    m_allocator->free(node);
                }

                inline leaf*   allocate_leaf()  throw()
                {
                    return reinterpret_cast<leaf*> ( m_allocator->allocate( sizeof(leaf) ) );
                }

                inline void free_leaf(leaf* leaf)  throw()
                {
                ...
                inline void register_tiny_pages(uintptr_t start, uintptr_t size, uintptr_t page_block_address) throw()
                {
					const size_t  page_size = 4096;
                    register_pages(start, static_cast<uint32_t> ( align(static_cast<size_t>(size), page_size) >> detail::log2_c<page_size>::value ), page_block_address);
                }

                inline void register_large_pages(uintptr_t start, uintptr_t size, uintptr_t block_size) throw()
                {
                    const size_t  page_size   = 4096;
                    register_pages(start, static_cast<uint32_t> ( align(static_cast<size_t>(size), page_size) >> detail::log2_c<page_size>::value ), encode_large_object(block_size) );
                }
                ...
                inline static bool is_large_object ( uintptr_t value ) throw()
                {
                    return value != decode_large_object(value);
                }

                inline static uintptr_t   encode_large_object( uintptr_t block_size) throw()
                {
                    return  (~0x7fffffffffffffffUL) | block_size;
                }

                inline static uintptr_t   decode_large_object( uintptr_t address) throw()
                {
