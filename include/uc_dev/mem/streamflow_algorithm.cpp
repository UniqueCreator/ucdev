...
namespace uc
{
    namespace mem
    {
        namespace streamflow
        {
            ...
            //---------------------------------------------------------------------------------------
            page_block* internal_heap::get_free_page_block(uint32_t size, thread_id thread_id) throw()
            {
            ...
            static void thread_finalize( internal_heap** heaps, uint32_t heap_count )
            {
                if (t_thread_local_heap_info != nullptr)
                {
                    for (uint32_t i = 0 ; i < heap_count; ++i)
                    {
                        ...
                        for (uint32_t j = 0; j < size_classes;++j)
                        {
                            thread_local_heap* local_heap = &local_heap_info->t_local_heaps[ j ] ;
            ...
            initialization_code initialize() throw()
            {
               ...
               if (heap_memory != nullptr && public_heaps_memory != nullptr)
               {
                    for (uint32_t i = 0; i < 8;++i)
                    {
                        ...
                        void*     addr = reinterpret_cast<void*> ( mem_private_heap );
                        heaps[i] = new (addr) mem::streamflow::internal_heap(i);
