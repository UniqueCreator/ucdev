...
namespace uc
{
    namespace model
    {
        ...
        inline auto joint_depths(const std::vector< gx::import::geo::joint > & d)
        {
            ...
            {
                uint32_t depth = 0;
                for (auto j = d[i].m_parent_index; j != -1; j = d[j].m_parent_index)
                {
                    depth++;
        ...
        inline auto joint_link(const gx::import::geo::skeleton_pose& s)
        {
            ...
            while (!joint_indices.empty())
            {
                ...
                while (!links.empty() && (simd_counter < max_split))
                {
                    ...
                    simd.m_linkages[simd_counter].m_joint = static_cast<uint16_t>(joint->m_joint);
                    simd.m_linkages[simd_counter].m_parent = static_cast<uint16_t>(joint->m_joint_parent);
        ...
        /*
        inline std::unique_ptr< uc::lip::skeleton > skeleton(const gx::geo::skeleton_pose& pose, uint16_t locomotion_index = 0)
        {
