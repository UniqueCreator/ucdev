...
#include <uc_public/graphics/window_overlay_swap_chain_resources.h>
...
namespace UniqueCreator
{
    namespace Graphics
    {
        class WindowOverlaySwapChainResources::Impl
        {
        public:
        ...
        namespace
        {
            static auto CreateSwapChain(IDXGIFactory2* factory, ID3D12CommandQueue* queue, const winrt::Windows::UI::Core::CoreWindow& window, uint32_t back_buffer_width, uint32_t back_buffer_height)
            {
                ...
                swapChainDesc.SwapEffect            = DXGI_SWAP_EFFECT_FLIP_DISCARD;                // for hud rendering we will use partial presentation optimizations
        ...
        WindowOverlaySwapChainResources::WindowOverlaySwapChainResources(ResourceCreateContext* ctx, const winrt::Windows::UI::Core::CoreWindow& window, const winrt::Windows::Graphics::Display::DisplayInformation& displayInformation) :
        Base( nullptr, ctx->CreateDirectQueue().release(), ctx)
        {
            m_impl->m_queue.reset(Base::m_impl->m_direct_queue);
            {
                ...
                Base::m_impl->m_resize_flags       = DXGI_SWAP_CHAIN_FLAG_FOREGROUND_LAYER;
        ...
        WindowOverlaySwapChainResources::~WindowOverlaySwapChainResources() = default;
        WindowOverlaySwapChainResources& WindowOverlaySwapChainResources::operator=(WindowOverlaySwapChainResources&&) = default;

        void WindowOverlaySwapChainResources::SetLogicalSize(Size2D size)
        {
        ...
        void WindowOverlaySwapChainResources::SetDisplayInformation(const winrt::Windows::Graphics::Display::DisplayInformation& displayInformation)
        {
            m_impl->m_display_information = displayInformation;
