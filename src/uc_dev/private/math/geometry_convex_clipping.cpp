...
namespace std
{
    ...
    struct hash< std::tuple<int32_t, int32_t> >
    {
        size_t operator()(const std::tuple<int32_t, int32_t> & l) const
        {
            ...
            size_t res = 17;
            auto[a, b] = l;
            res = res * 31 + hash<int32_t>()(a);
            res = res * 31 + hash<int32_t>()(b);
            return res;
        }
    };
}
        ...
        namespace convex_clipping
        {
            namespace
            {
                struct closed_convex_clipper
                {
                    ...
                    std::vector<float4>                     m_vertices_points;
                    ...
                    std::tuple<int32_t, int32_t> process_vertices(const plane& p)
                    {
                        ...
                        for (auto i = 0U; i < vertices_to_process; ++i)
                        {
                            ...
                            if (v.m_visible)
                            {
                                v.m_distance = get_x(dot4(p.m_value, v_point));
                    ...
                    void process_edges()
                    {
                        auto edges_to_process = static_cast<int32_t>(m_edges.size());

                        for (auto i = 0; i < edges_to_process; ++i)
                        {
                            auto&& e = m_edges[i];
                            ...
                            if (e.m_visible)
                            {
                                ...
                                float t = d0 / (d0 - d1);
                                float4 point = lerp(v0_point, v1_point, t);
                    ...
                    void process_faces(const plane& clip_plane)
                    {
                        ...
                        for (auto i = 0U; i < faces_to_process; ++i)
                        {
                            ...
                            if (f.m_visible)
                            {
                                ...
                                auto[is_open, start, end] = get_open_polyline(f);
                    ...
                    float4 get_normal(const std::vector<int32_t>& vi)
                    {
                        ...
                        {
                            normal = add(normal, cross3(m_vertices_points[i], m_vertices_points[i + 1]));
                        }
                    ...
                    std::vector<int32_t> get_ordered_faces()
                    {
                        ...
                        for (auto i = 0U; i < faces_to_process; ++i)
                        {
                            ...
                            if (f.m_visible)
                            {
                                ...
                                if (get_x(dot3(m_faces[i].m_plane.m_value, get_normal(vertices))) > 0.0f)
                                {
                                    //counterclockwise
                    ...
                    int32_t     clip(const plane& clipPlane)
                    {
                        //vertex processing
                        {
                            auto [negative, positive ] = process_vertices(clipPlane);
                    ...
                    std::tuple< std::vector<float4>, std::vector<int32_t> > convert()
                    {
                ...
                template <>
                closed_convex_clipper make_clipper<frustum_points>(const frustum_points& b)
                {
...

                    {
                        ...
                        for (auto&& p : b.m_points)
                        {
                            r.m_vertices_points.push_back(p);
                        }
                    }
                ...
                closed_convex_clipper make_clipper(const convex_polyhedron& b)
                {
                    ...
                    //build planes
                    {
                        ...
                        for (auto i = 0U; i < r.m_faces.size(); ++i)
                        {
                            ...
                            const float4 a0     = r.m_vertices_points[e0.m_vertices[0]];
                            const float4 b0     = r.m_vertices_points[e0.m_vertices[1]];
                            const auto   index  = e0.m_vertices[0] == e1.m_vertices[0] ? 1 : 0;
                            const float4 c0     = r.m_vertices_points[e1.m_vertices[index]];

                            r.m_faces[i].m_plane = make_plane(a0, b0, c0);
                        ...
                        for (auto i = 0U; i < r.m_faces.size(); ++i)
                        {
                            ...
                            for (auto&& v : r.m_vertices_points)
                            {
                                ...
                                {
                                    r.m_faces[i].m_plane.m_value = mul( plane.m_value, set(-1.0f, -1.0f, -1.0f, 1.0f) );
                                    break;
                                }
                            }
                        }
